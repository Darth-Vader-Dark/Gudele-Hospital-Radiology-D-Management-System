        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0"><i class="fas fa-calendar-edit"></i> Edit Appointment</h4>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('registration.appointment.update', $appointment->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('scheduled_date') is-invalid @enderror" 
                                           ...
                                           value="{{ old('scheduled_date', $appointment->scheduled_date->format('Y-m-d\TH:i')) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('reason_for_visit') is-invalid @enderror" 
                                      id="reason_for_visit" name="reason_for_visit" rows="2">{{ old('reason_for_visit', $appointment->reason_for_visit) }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="scheduled" @if(old('status', $appointment->status) === 'scheduled') selected @endif>Scheduled</option>
                                        ...
                                        <option value="no-show" @if(old('status', $appointment->status) === 'no-show') selected @endif>No-show</option>
                                    </select>
                                    @error('status')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
